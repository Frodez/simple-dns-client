...
std::optional<std::vector<std::string>> dns_cache_manager::get(const std::string& domain)
{
...
void dns_cache_manager::refresh(std::unique_ptr<dns_msg> msg)
{
    if (msg->header.get_rcode() != 0 || msg->query.empty() || msg->answer.empty()) {
        return;
    }
...
std::unordered_map<std::string, std::vector<std::string>> dns_cache_manager::parse_dns_msg(std::unique_ptr<dns_msg> msg)
{

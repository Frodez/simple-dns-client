...
struct dns_header {
    ...
    std::string to_string()
    {
        ...
        return fmt::format("[id={:d}, flags=[raw={:#x}, qr={:#x}, opcode={:#x}, aa={:#x}, tc={:#x}, "
                           "rd={:#x}, ra={:#x}, zero={:#x}, rcode={:#x}], qdcount={:d}, ancount={:d}, "
                           "nscount={:d}, arcount={:d}]",
    ...
    std::string to_json_string()
    {
        ...
        uint16_t zero = get_zero();
        uint16_t rcode = get_rcode();
        return fmt::format("{{\"id\":{:d}, \"flags\":{{\"raw\":\"{:#x}\", \"qr\":\"{:#x}\", "
                           "\"opcode\":\"{:#x}\", \"aa\":\"{:#x}\", \"tc\":\"{:#x}\", \"rd\":\"{:#x}\", "
                           "\"ra\":\"{:#x}\", \"zero\":\"{:#x}\", \"rcode\":\"{:#x}\"}}, \"qdcount\":{:d}, "
...
private:
    ...
    {
        //about the compress flag, you can see:
        //https://tools.ietf.org/html/rfc1035 (in the 4.1.4. Message compression)
    ...
    static std::tuple<std::string, size_t> read_string(uint8_t* buf, size_t offset, str_table& table)
    {
        ...
        while (true) {
            if (is_compress_flag(begin + now)) {
                //compressed string must be the last part of the string,
                //or the whole part of the string.
...
public:
    ...
    {
        //about the ipv4 address, you can see:
        //https://tools.ietf.org/html/rfc1035 (in the 3.3.12. PTR RDATA format)
        //about the ipv6 address, you can see:
    ...
    {
        if (header.get_rcode() == 0) {
            return {};
        } else if (header.get_rcode() == 1) {
            ...
            return fmt::format("Unknown error - code={:d}", header.get_rcode());
        }
    }
    std::string to_string()
    {
